...
function JwtRedis(client, options) {
  ...
  this.prefix = options && options.prefix ? options.prefix : 'session:';
  this.expiresKeyIn = options && options.expiresKeyIn ? options.expiresKeyIn : null;
  ...
  this.sign = (payload, secret, options) => {
    options = cloneDeep(options) || {};
    payload = cloneDeep(payload) || {};
    const expiresIn = options.expiresIn || payload.exp;
    const expiresKeyIn = options.expiresKeyIn || this.expiresKeyIn;
  ...
  this.verify = (token, secret, options) => {
    const decoded = await this.decode(token);
    if (decoded.jti) {
